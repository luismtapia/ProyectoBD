package capacitaciones;

import java.net.URL;
import java.sql.SQLException;
import java.util.List;
import java.util.ResourceBundle;
import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.fxml.Initializable;
import javafx.scene.control.Alert;
import javafx.scene.control.Button;
import javafx.scene.control.ContentDisplay;
import javafx.scene.control.Label;
import javafx.scene.control.ListView;
import javafx.scene.control.TextField;
import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.paint.Color;
import SQL.SQL_Actualizar;
import SQL.SQL_Consultar;
import SQL.SQL_Crear;
import SQL.SQL_Eliminar;
import java.sql.ResultSet;
import java.util.Optional;
import javafx.scene.control.ButtonType;
import javafx.scene.control.ComboBox;

public class CapacitacionesController implements Initializable{
    SQL_Crear con_crear= new SQL_Crear();
    SQL_Actualizar con_actualizar= new SQL_Actualizar();
    SQL_Consultar con_consultar= new SQL_Consultar();
    SQL_Eliminar con_Eliminar= new SQL_Eliminar();
    int no_capacitacion_anterior;
    
    @FXML
    private TextField nombre_capacitacion,nombre_capacitador;
    @FXML
    private Button btn_crear,btn_leer,btn_eliminar,btn_actualizar,btn_aceptar_crear,btn_aceptar_actualizar,btn_aceptar_eliminar,btn_seleccionar,btn_cancelar;
    @FXML
    private Label lbl1, lbl2,lbl3;
    @FXML
    private ComboBox cmb_area;
    @FXML
    private ListView lista_capacitaciones;
    @FXML
    private void handleBotones(ActionEvent event) throws SQLException{
        if(event.getSource() == btn_crear){
            vista_consultar(false);
            vista_actualizar(false);
            vista_eliminar(false);
            vista_crear(true);
            btn_aceptar_actualizar.setVisible(false);
            btn_cancelar.setVisible(false);
            llenarcomboboxArea();
            limpiar();
        }else
            if(event.getSource() == btn_leer){
                vista_crear(false);
                vista_actualizar(false);
                vista_eliminar(false);
                vista_consultar(true);
                btn_cancelar.setVisible(false);
                btn_aceptar_actualizar.setVisible(false);
                busqueda();
                limpiar();
            }else
                if(event.getSource() == btn_actualizar){
                    vista_crear(false);
                    vista_consultar(false);
                    vista_eliminar(false);
                    vista_actualizar(true);
                    btn_cancelar.setVisible(false);
                    btn_aceptar_actualizar.setVisible(false);
                    limpiar();
                    llenarcomboboxArea();
                    busqueda();
                }else
                    if(event.getSource() == btn_eliminar){
                        vista_crear(false);
                        vista_consultar(false);
                        vista_actualizar(false);
                        vista_eliminar(true);
                        busqueda();
                        btn_cancelar.setVisible(false);
                        btn_aceptar_actualizar.setVisible(false);
                    }else
                        if(event.getSource() == btn_seleccionar){
                            seleccionar();
                            btn_seleccionar.setVisible(false);
                            btn_cancelar.setVisible(true);
                            btn_aceptar_actualizar.setVisible(true);
                            seleccionar();
                            //
                        }else
                        if(event.getSource() == btn_cancelar){
                            btn_seleccionar.setVisible(true);
                            btn_cancelar.setVisible(false);
                            btn_aceptar_actualizar.setVisible(false);
                            limpiar();
                        }else
                            if(event.getSource() == btn_aceptar_actualizar){
                                actualizar();
                                busqueda();
                                btn_aceptar_actualizar.setVisible(false);
                                limpiar();
                            }else
                                if(event.getSource() == btn_aceptar_crear){
                                    crear();
                                }else
                                    if(event.getSource() == btn_aceptar_eliminar){
                                        eliminar();
                                        busqueda();
                                    }
    }
    
    @Override
    public void initialize(URL url, ResourceBundle rb){
        color(Color.web("#D4AF37"));
        vista_crear(false);
        vista_consultar(false);
        vista_actualizar(false);
        vista_eliminar(false);
        btn_cancelar.setVisible(false);
        btn_aceptar_actualizar.setVisible(false);
        imagenes();
        nombre_capacitacion.setPromptText("Nombre de la capacitación");
        nombre_capacitador.setPromptText("Nombre del capacitador");
    }
    
    private void color(Color color){
        btn_crear.setTextFill(color);
        btn_leer.setTextFill(color);
        btn_actualizar.setTextFill(color);
        btn_eliminar.setTextFill(color);
        
        btn_seleccionar.setTextFill(color);
        btn_cancelar.setTextFill(color);
        
        btn_aceptar_crear.setTextFill(color);
        btn_aceptar_actualizar.setTextFill(color);
        btn_aceptar_eliminar.setTextFill(color);
        
        lbl1.setTextFill(color);
        lbl2.setTextFill(color);
        lbl3.setTextFill(color);
    }
    
    private void vista_crear(boolean vista){
        lbl1.setVisible(vista);
        lbl2.setVisible(vista);
        lbl3.setVisible(vista);
        cmb_area.setVisible(vista);
        nombre_capacitacion.setVisible(vista);
        nombre_capacitador.setVisible(vista);
        btn_aceptar_crear.setVisible(vista);
    }
    
    private void vista_consultar(boolean vista){
        lista_capacitaciones.setVisible(vista);
    }
    
    private void vista_actualizar(boolean vista){
        lbl1.setVisible(vista);
        lbl2.setVisible(vista);
        lbl3.setVisible(vista);
        cmb_area.setVisible(vista);
        nombre_capacitacion.setVisible(vista);
        nombre_capacitador.setVisible(vista);
        btn_seleccionar.setVisible(vista);
        lista_capacitaciones.setVisible(vista);
        //btn_aceptar_actualizar.setVisible(vista);
    }
    
    private void vista_eliminar(boolean vista){
        lista_capacitaciones.setVisible(vista);
        
        btn_aceptar_eliminar.setVisible(vista);
    }
    
    private void imagenes(){
        btn_crear.setGraphic(new ImageView(new Image("iconos/crear.png")));
        btn_crear.setContentDisplay(ContentDisplay.BOTTOM);
        btn_leer.setGraphic(new ImageView(new Image("iconos/consultar.png")));
        btn_leer.setContentDisplay(ContentDisplay.BOTTOM);
        btn_actualizar.setGraphic(new ImageView(new Image("iconos/actualizar.png")));
        btn_actualizar.setContentDisplay(ContentDisplay.BOTTOM);
        btn_eliminar.setGraphic(new ImageView(new Image("iconos/eliminar.png")));
        btn_eliminar.setContentDisplay(ContentDisplay.BOTTOM);
    }
    
    private void busqueda() throws SQLException{
        lista_capacitaciones.getItems().clear();
        lista_capacitaciones.setItems(con_consultar.getCapacitacion());
    }
    
    private void llenarcomboboxArea(){
        cmb_area.getItems().clear();
        cmb_area.setPromptText("SELECCIONE EL AREA");
        ResultSet rs = con_consultar.consultarArea();
        try {
            while (rs.next()) {                
                cmb_area.getItems().add(rs.getString("nombre_area"));
            }
        } catch (Exception e) {
            System.out.println(e);
        }
    }
    
    private void seleccionar(){
        List<Capacitacion> sval = lista_capacitaciones.getSelectionModel().getSelectedItems();
            for(int i=0;i<sval.size();i++){
                Capacitacion capacitacionSeleccionada = sval.get(i);
                try{
                    no_capacitacion_anterior=capacitacionSeleccionada.getNo_capacitacion();
                    String nombre_capacitacion_=capacitacionSeleccionada.getNombre_capacitacion();
                    String nombre_capacitador_=capacitacionSeleccionada.getNombre_capacitador();
                    int cve_area=capacitacionSeleccionada.getCve_area();
                    
                    ResultSet rs=con_consultar.getAreaPorClave(cve_area);
                    String _area="";
                    while (rs.next()) {                        
                        _area=rs.getString("nombre_area");
                    }
                    
                    nombre_capacitacion.setText(nombre_capacitacion_);
                    nombre_capacitador.setText(nombre_capacitador_);
                    cmb_area.setValue(_area);
                }
                catch(Exception e){
                    System.out.println(""+e);
                }
            }
    }
    
    private void actualizar() throws SQLException{
        String nombre_capacitacion_=nombre_capacitacion.getText();
        String nombre_capacitador_=nombre_capacitador.getText();
        
        String area=String.valueOf(cmb_area.getSelectionModel().getSelectedItem());
        int cve_area=0;
        ResultSet rs=con_consultar.getAreaPorNombre(area);
        while (rs.next()) {                
            cve_area=rs.getInt("cve_area");
        }
        if(con_actualizar.ActualizaCapacitacion(no_capacitacion_anterior,nombre_capacitacion_,nombre_capacitador_,cve_area)){
            Alert m=new Alert(Alert.AlertType.INFORMATION);
            m.setTitle("Capacitacion");
            m.setContentText("Capacitacion actualizado correctamente");
            m.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
            m.show();
        }else{
            Alert m=new Alert(Alert.AlertType.INFORMATION);
            m.setTitle("Capacitacion");
            m.setContentText("Capacitacion no pudo ser actualizado");
            m.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
            m.show();
        }
    }
    
    private void limpiar(){
        nombre_capacitador.setText("");
        nombre_capacitacion.setText("");
    }
    
    private void crear(){
        String nombre_area_="";
        try {
            nombre_area_=String.valueOf(cmb_area.getSelectionModel().getSelectedItem());
            int cve_area_=-1;
            
            ResultSet rs = con_consultar.getAreaPorNombre(nombre_area_);
            try {
                while (rs.next()) {                
                    cve_area_=rs.getInt("cve_area");
                }
            } catch (Exception e) {
                System.out.println(e);
            }
            
            
            if(!nombre_area_.equals("SELECCIONE EL AREA")){
                    Capacitacion nuevacapacitacion= new Capacitacion(0, nombre_capacitacion.getText(),  nombre_capacitador.getText(), cve_area_);
                    if(con_crear.insertaCapacitacion(nuevacapacitacion)){
                       Alert msg=new Alert(Alert.AlertType.INFORMATION);
                       msg.setTitle("Registro guardado");
                       msg.setContentText("guardado correctamente");
                       msg.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
                       msg.show();
                    }
                }else{
                    Alert m=new Alert(Alert.AlertType.INFORMATION);
                    m.setTitle("ERROR");
                    m.setContentText("SELECCIONE UN AREA POR FAVOR");
                    m.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
                    m.show();
                }
                
            } catch (Exception e) {    
                Alert m=new Alert(Alert.AlertType.INFORMATION);
                m.setTitle("Capacitacion");
                m.setContentText("Capacitacion no pudo ser creada"+e);
                m.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
                m.show();
                System.out.println(e);
            }
    }
    
    private void eliminar(){
        List<Capacitacion> sval = lista_capacitaciones.getSelectionModel().getSelectedItems();
            for(int i=0;i<sval.size();i++){
                Capacitacion capacitacionSeleccionada = sval.get(i);
                try{
                    Alert msg =new Alert(Alert.AlertType.CONFIRMATION);
                    msg.setTitle(("ELiminar"));
                    msg.setHeaderText("Capacitacion");
                    msg.setContentText("¿Seguro de eliminar la capacitacion seleccionada?\n\n"+capacitacionSeleccionada.getNombre_capacitacion());
                    msg.setGraphic(new ImageView(new Image("iconos/capacitacion.png")));
                    Optional<ButtonType> res= msg.showAndWait();
                    if(res.get()==ButtonType.OK)
                    {
                        if(con_Eliminar.eliminaCapacitacion(capacitacionSeleccionada.getNo_capacitacion())){
                            Alert ms =new Alert(Alert.AlertType.CONFIRMATION);
                            ms.setTitle(("Area"));
                            ms.setHeaderText("Eliminada");
                            ms.setContentText("Se ha eliminado la capacitacion "+ capacitacionSeleccionada.getNo_capacitacion()+ " "+capacitacionSeleccionada.getNombre_capacitacion() + " correctamente");
                            ms.setGraphic(new ImageView(new Image("iconos/IconosMenu/empleado.png")));
                        }
                    }
                }
                catch(Exception e){
                    System.out.println(e.getMessage());
                }
            }
    }
}